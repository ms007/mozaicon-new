...
const STATUS_OPTIONS = [
  ...
  { id: 'opt-ready-to-merge', name: 'Ready to Merge' },
...
const CTX: ProjectContext = {
  ...
  statusOptions: {
    ...
    'Ready to Merge': 'opt-ready-to-merge',
    Done: 'opt-done',
  },
}
...
describe('hasStatusSchema', () => {
  ...
  it('rejects a project missing one of Todo/In Progress/In Review/Ready to Merge/Done', () => {
  ...
  it('ignores extra status options beyond the five canonical ones', () => {
...
describe('mapStatusOptions', () => {
  ...
  it('throws if a required name is missing', () => {
    ...
    assert.throws(() => mapStatusOptions(partial), /missing Status option "Ready to Merge"/)
  })
...
describe('toRelatedIssue', () => {
  ...
  it('preserves issue state (OPEN/CLOSED) so the planner can detect closed work', () => {
    const open = toRelatedIssue(
    ...
    const closed = toRelatedIssue(
      makeRelated({ number: 2, statusOptionId: 'opt-todo', state: 'CLOSED' }),
      CTX,
      STUB_LOOKUP,
    )
    ...
    assert.equal(
      closed.eligible,
      false,
      'a CLOSED issue must never be eligible — even if its Status is still Todo',
    )
  })

...
describe('execute — individual action types', () => {
  ...
  it('runReviewer with approved verdict advances phase to reviewed and promotes status to Ready to Merge', async () => {
    const { deps, log } = fakeActionDeps()
    ...
    assert.deepEqual(log, ['runReviewer(1)', 'moveStatus(item-1, Ready to Merge)'])
  })

  it('runReviewer with approved verdict skips moveStatus when itemId is null', async () => {
    const { deps, log } = fakeActionDeps()
    const action: Action = { tag: 'runReviewer', issue: issue(1, null) }
    const next = await execute(action, emptyState, deps)
    assert.equal(next.state.phases.get(1), 'reviewed')
...
describe('runWorkflow — recovery from prior run via initialPhases', () => {
  it("does not re-implement a leaf seed whose recovered phase is 'promoted' (In Review)", async () => {
    // Reproduces: a single-issue run that was promoted to In Review on a
    // prior invocation. Restart must run the reviewer next, NOT re-claim
    // and re-implement.
    const config: WorkflowConfig = {
      seed: { ...issue(1), isPrd: false },
      children: [],
    ...
    const initialPhases = new Map<number, IssuePhase>([[1, 'promoted']])
    const { deps, log } = fakeActionDeps()
    const result = await runWorkflow(
      config,
      { observe: noopObserveDeps, actions: deps },
      initialPhases,
    )
    assert.equal(result.tag, 'done')

    assert.ok(
      !log.includes('runImplementer(1)'),
      "An issue recovered as 'promoted' must not be re-implemented on restart.",
    )
    assert.ok(
      !log.includes('moveStatus(item-1, In Progress)'),
      "An issue recovered as 'promoted' must not be re-claimed (status reset to In Progress) on restart.",
    )
    assert.ok(log.includes('runReviewer(1)'))
  })

  it("does not re-review a leaf seed whose recovered phase is 'reviewed' (Ready to Merge)", async () => {
    // Reproduces: a single-issue run where the reviewer approved on a prior
    // invocation (Ready to Merge). Restart must skip review and run merger.
    const config: WorkflowConfig = {
      seed: { ...issue(1), isPrd: false },
      children: [],
      tickCap: 50,
    ...
    const initialPhases = new Map<number, IssuePhase>([[1, 'reviewed']])
    const { deps, log } = fakeActionDeps()
    const result = await runWorkflow(
      config,
      { observe: noopObserveDeps, actions: deps },
    ...
    assert.ok(!log.includes('runImplementer(1)'))
    assert.ok(!log.includes('runReviewer(1)'))
    assert.ok(log.includes('runMerger([1])'))
  })

  it("does not re-implement a leaf seed whose recovered phase is 'done'", async () => {
    // Reproduces: a single-issue run that finished on a prior invocation
    // (project Status = Done). Restart must NOT re-claim or re-implement it.
    const config: WorkflowConfig = {
      seed: { ...issue(1), isPrd: false },
      children: [],
      tickCap: 50,
    ...
    const initialPhases = new Map<number, IssuePhase>([[1, 'done']])
    const { deps, log } = fakeActionDeps()
    const result = await runWorkflow(
      config,
      { observe: noopObserveDeps, actions: deps },
    ...
    assert.deepEqual(
      log,
      [],
      "A leaf issue recovered as 'done' must not trigger any actions on restart.",
    )
  })

  it("does not re-implement PRD children whose recovered phase is 'done'", async () => {
    // Reproduces: a PRD whose first child shipped (Done) before a crash.
    // Restart must skip child 1 entirely and continue with child 2.
    const config: WorkflowConfig = {
      seed: { ...issue(100), isPrd: true },
      children: [issue(1), issue(2)],
      tickCap: 50,
    ...
    assert.ok(
      !log.includes('runImplementer(1)'),
      "Child recovered as 'done' must not be re-implemented on restart.",
    )
    assert.ok(
      !log.includes('moveStatus(item-1, In Progress)'),
      "Child recovered as 'done' must not be re-claimed on restart.",
    )
    // Child 2 still progresses normally
    assert.ok(log.includes('runImplementer(2)'))
  })

  it("does not re-review PRD children whose recovered phase is 'reviewed' (Ready to Merge)", async () => {
    // Reproduces: every child reached Ready to Merge before the crash.
    // Restart must skip review and merger should pick them all up directly.
    const config: WorkflowConfig = {
      seed: { ...issue(100), isPrd: true },
      children: [issue(1), issue(2)],
      tickCap: 50,
    ...
    const initialPhases = new Map<number, IssuePhase>([
      [1, 'reviewed'],
      [2, 'reviewed'],
    ])
    const { deps, log } = fakeActionDeps()
    ...
    assert.ok(!log.some((l) => l.startsWith('runImplementer')))
    assert.ok(!log.some((l) => l.startsWith('runReviewer')))
    assert.ok(log.includes('runMerger([1,2])'))
  })

  it("recovers a mixed PRD with one child 'done' and another 'reviewed' without redoing earlier work", async () => {
    // The scenario from the bug report: a multi-child PRD aborted partway —
    // one child finalized (Done), another sitting at Ready to Merge, a third
    // still in flight (In Review). Restart must NOT re-implement or re-review
    // the first two; only the third needs review.
    const config: WorkflowConfig = {
      seed: { ...issue(100), isPrd: true },
      children: [issue(1), issue(2), issue(3)],
      tickCap: 50,
    ...
    assert.ok(!log.includes('runImplementer(1)'))
    assert.ok(!log.includes('runImplementer(2)'))
    assert.ok(!log.includes('runImplementer(3)'))
    assert.ok(!log.includes('runReviewer(1)'))
    assert.ok(!log.includes('runReviewer(2)'))
    // Only the In-Review child needs the reviewer to run
    assert.ok(log.includes('runReviewer(3)'))
    // Issue 1 already shipped — its only remaining lifecycle event is none.
    assert.ok(!log.includes('moveStatus(item-1, Done)'))
  })
})
